...
The Model Context Protocol (MCP) enables MindStack to connect with diverse data sources and tools through a standardized interface. MindStack features an intelligent two-stage MCP approach that automatically selects the best tools and generates contextual research, powered by LangChain's [MCP adapters](https://github.com/langchain-ai/langchain-mcp-adapters) for seamless integration.

## How MCP Works in MindStack

MindStack uses a **two stage intelligent approach** for MCP integration:
...
3. **Initialization**: MindStack loads your `mcp_configs`
...
MCP support is included with MindStack installation:
...
**Important:** To use MCP with MindStack, you must set the `RETRIEVER` environment variable:
...
# MindStack automatically orchestrates all servers
context = await researcher.conduct_research()
report = await researcher.write_report()
...
This example demonstrates how MindStack intelligently:
...
MindStack automatically detects connection types:
...
*For more examples and advanced use cases, check out the [MindStack examples repository](https://github.com/assafelovic/gpt-researcher/tree/master/examples).* :-) 